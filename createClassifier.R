...
#'
#'This function is called by \code{\link{trainClassifier}} and is a wrapper for calling
#'\code{\link{subsetComparison}} prior to a 'typeK' based decision. If 'typeK' equals 
#'"range", then \code{\link{scanClassifierRangeK}} is called and produces classifier
...
#'@param comp_type integer of value 1, 2, 3, 4, 5, or 6 or a character vector of 2
#' elements that specifies the groups of samples to keep in the data frame (default: 4).
...
#'@examples
#'\dontrun{
#'func_output_1 <- passDataToCreateClassifier(data, 1, "range", c(250, 90, 110), 1)
...
passDataToCreateClassifier <- function(data, comp_type, typeK, range_pars, remove_overlaps=TRUE, 
  ...
  classifierargs <- c("classifier_name_base", "classifier_name", "dv")
  
  if (typeK == "range") {
    createClassOut <- do.call(scanClassifierRangeK, c(list(data_subset=data_subset, range_pars=range_pars, type=type, 
                                                           comp_type=comp_type), dots[names(dots) %in% c("dv", svmargs)]))
  } else {
    ...
    # Report the median LOOCV accuracy of the sc solutions
    cvacc <-  cv
