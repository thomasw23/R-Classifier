...
testClassifier <- function(obj, train_data_name, data, type, dv, sub_name="",
...
testClassifier.default <- function(obj, train_data_name, data, type, dv, sub_name="", 
                                   svm_cost, svm_gamma, ..., train_data, a=0, b=100) {
  fc <- unlist(strsplit(train_data_name, split="v"))[1]
  dots <- list(...)
  discretize_args <- c("discretize_name_base", "discretize_name") 
  disc_data <- do.call(filterTestData, c(list(d=data, disc_feat=colnames(train_data)[-1], 
                                                  data_name=train_data_name, type=type, dv=dv), 
...
testClassifier.txClassifier <- function(obj, train_data_name, data, type, dv, sub_name="", 
                                        svm_cost, svm_gamma, ..., train_data, a=0, b=100) {
  
  if(sub_name == "") {
    dots <- list(...)
    discretize_args <- c("discretize_name_base", "discretize_name") 
    disc_data <- do.call(filterTestData, c(list(d=data, disc_feat=obj$probes, 
                                                    data_name=train_data_name, type=type, dv=dv), 
                                               dots[names(dots) %in% discretize_args]))
...
testClassifier.data.frame <- function(obj, train_data_name, data, type, dv, sub_name="", 
                                      svm_cost, svm_gamma, ..., train_data, a=0, b=100) {
